...
void observarRetornoFisico() {
  ...
  // O motor já estava confirmado ligado e perdeu o retorno sem existir
  // um comando de desligamento. Desarma o relé para impedir que o motor
  // volte sozinho quando a chave/manual/proteção for restabelecida.
  if (!atual && estadoRele && motorConfirmadoLigado && !aguardandoRetorno) {
    Serial.println("Retorno perdido durante funcionamento. Desarmando rele por seguranca.");
    controlarRele(false);
...
</style></head><body><div class="topo"><h1>Quatrin AquaControl</h1><p>Configuração de rede</p></div><div class="c"><h2>Conectar ao Wi-Fi</h2><form method="POST" action="/salvar"><label>Rede Wi-Fi</label><select name="ssid" required>
)rawliteral";
...
void atualizarConfirmacaoFisica() {
  ...
  if (estadoReal == estadoEsperadoRetorno) {
    ...
    if (millis() - inicioRetornoCorreto >= TEMPO_ESTABILIZAR_RETORNO) {
      if (estadoEsperadoRetorno) motorConfirmadoLigado = true;
      else motorConfirmadoLigado = false;
  ...
  if (inicioAguardandoRetorno > 0 &&
    ...
    // Em qualquer falha de confirmação o comando remoto é retirado.
    controlarRele(false);
    motorConfirmadoLigado = false;
...
bool enviarPing() {
  ...
  if (comandoSeq > ultimoComandoRecebido &&
    ...
    // Qualquer novo comando explícito libera o bloqueio anterior.
...
void reciclarConexaoWifi() {
  ...
  unsigned long inicio = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - inicio < 6000) delay(100);
...
void loop() {
  ...
  if (solicitarReciclagemWifi) reciclarConexaoWifi();
