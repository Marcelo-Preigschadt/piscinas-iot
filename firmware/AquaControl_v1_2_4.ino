...
// A partir da 1.2.4 o estado do motor é determinado pelo retorno físico
// do GPIO 27. Com INPUT_PULLUP: LOW = contactora/motor confirmado.
...
const unsigned long INTERVALO_PING_FALHA = 2000;
...
// Comando recebido do servidor e confirmação física.
...
// ======================================================
//           RESET BOOT INDEPENDENTE DO LOOP/HTTPS
// ======================================================
...
void tarefaMonitorBoot(void* parametro) {
  ...
  for (;;) {
    ...
    // Arma somente depois de detectar o BOOT solto. Evita reset acidental
    // durante upload ou reinicialização da placa.
    if (!pressionado) {
      armado = true;
      inicioPressionado = 0;
    } else if (armado) {
      ...
      if (millis() - inicioPressionado >= TEMPO_RESET_WIFI) {
        ...
        // Estado seguro antes do reinício.
        estadoRele = false;
    ...
    // Leitura independente a cada 10 ms. A tarefa tem prioridade maior que
    // o loop e continua executando mesmo durante chamadas HTTP/TLS.
...
String valorJson(const String& json, const String& chave) {
  ...
  if (i >= json.length() || json.substring(i, i + 4) == "null" || json[i] != '"') {
    return "";
  }
...
void registrarDispositivo() {
  ...
  String ssidAtual = escaparJSON(WiFi.SSID());
...
void atualizarConfirmacaoFisica() {
  ...
  if (estadoReal == estadoEsperadoRetorno) {
    if (inicioRetornoCorreto == 0) {
      inicioRetornoCorreto = millis();
    }
...
void enviarPing() {
  ...
  if (comandoSeq > ultimoComandoRecebido &&
    ...
    // Não envia ACK neste ponto. O ACK só será enviado depois que o GPIO 27
    // confirmar fisicamente o estado solicitado.
  }
...
void setup() {
  ...
  // Inicia o reset do BOOT antes de Wi-Fi e HTTPS.
...
void loop() {
  // A confirmação física é observada continuamente, não apenas no ping.
  atualizarConfirmacaoFisica();
